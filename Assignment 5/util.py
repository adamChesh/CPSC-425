...
def build_vocabulary(image_paths, vocab_size):
    ...
    for i, path in enumerate(image_paths):
        ...
        # create a random list of numbers of size n_each to use as indices
        randomNumList = random.sample(range(0, len(sift_descriptors)), n_each)
        imageWereOn = i * n_each

        # add the randomly selected descriptors to the descriptors array for each image
        for j in range(n_each):
            descriptors[imageWereOn + j] = sift_descriptors[randomNumList[j], :]
    ...
    # fit the kmeans model using the randomly selected descriptors from above and return it
...
def get_bags_of_sifts(image_paths, kmeans):
    ...
    numOfBins = np.arange(vocab_size + 1)
    ...
    for i, path in enumerate(image_paths):
        ...
        # use the kmean model to make a prediction using the siftDescriptors
        siftDescriptors = features[:, 2:]
        predictedVals = kmeans.predict(siftDescriptors)

        # TODO: Build a histogram normalized by the number of descriptors

        # build a histogram with the predicted values, normalize results, add it to image_feats array
        imageHistogram, imageBins = np.histogram(predictedVals, numOfBins)
        imageHistogram = imageHistogram / siftDescriptors.shape[0]
        image_feats[i, :] = imageHistogram
